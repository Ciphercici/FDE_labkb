...
| | **方案 A：混合架构（本项目默认，实测）** | **方案 B：国内服务器单机版** |
|---|---|---|
| 拓扑 | 海外服务器收消息 → Git 队列 → 国内云函数抓取 | 一台国内服务器全包（收 + 抓 + 入库） |
| 需要 | 海外服务器 + Git 私有仓库 + 云函数 | 国内服务器（轻量即可，约 50~150 元/年） |
| 优点 | 云端全自动、按量付费零固定成本 | 架构最简单，无需队列和云函数；国内 IP 抓取最稳 |
| 缺点 | 组件多；云函数机房 IP 偶发风控 | 国内服务器需实名；QQ 登录仍建议专用小号 |
| 备案 | 无需 | 仅跑长连接/API、无对外 Web 服务 → **无需 ICP 备案** |

有国内服务器 → 方案 B 一步到位；没有 → 方案 A（本文档默认流程）。两种方案的详细部署差异见 [`docs/部署SOP.md`](docs/部署SOP.md)。
